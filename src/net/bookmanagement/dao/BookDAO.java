...
public class BookDAO {
	...
	public Boolean checkBookExisted(String bookName, String categoryName, List<String> authorNames ) {
		...
		try{
			...
			List<Book> book = session.createQuery(""
					+ "from Book b "
					+ "JOIN FETCH b._authors a "
					+ "JOIN FETCH b._category c "
					+ "where book_name=:bookName "
					+ "and category_name=:categoryName "
					+ "and (" + conditionOfAuthor.substring(3) +")").setParameter("bookName", bookName).setParameter("categoryName", categoryName).getResultList();
			
			if (book.size() > 0) checkExisted = true;
			transaction.commit();
